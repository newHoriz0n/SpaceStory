	...
	private static boolean computeCollision(Collidable c0, Collidable c1) {
		if (c0.getTeam() != c1.getTeam()) {
			...
			if (l_dist <= c0.getRadius() + c1.getRadius()) {
				...
				c0.getTraeger().getPosition().setKoordinaten(c0.getPosition().getX() - l_nx * l_overlap * (c1.getMasse() / l_sm1m2) * (1 + EPSILON),
						c0.getPosition().getY() - l_ny * l_overlap * (c1.getMasse() / l_sm1m2) * (1 + EPSILON), 0);
				c1.getTraeger().getPosition().setKoordinaten(c1.getPosition().getX() + l_nx * l_overlap * (c0.getMasse() / l_sm1m2) * (1 + EPSILON),
				...
				double l_sn1 = l_nx * c0.getSpeed().getX() + l_ny * c0.getSpeed().getY();
				double l_st1 = l_tx * c0.getSpeed().getX() + l_ty * c0.getSpeed().getY();
				...
				double l_sn2 = l_nx * c1.getSpeed().getX() + l_ny * c1.getSpeed().getY();
				double l_st2 = l_tx * c1.getSpeed().getX() + l_ty * c1.getSpeed().getY();
				...
				if (!c0.getTraeger().isFixiert()) {
					c0.getTraeger().getSpeed().setKoordinaten(l_vspx - l_n1x + l_t1x, l_vspy - l_n1y + l_t1y, 0);
				}
				if (!c1.getTraeger().isFixiert()) {
					c1.getTraeger().getSpeed().setKoordinaten(l_vspx - l_n2x + l_t2x, l_vspy - l_n2y + l_t2y, 0);
				}
				return true;
			}
		}
